...
| --- | --- | --- |
| **Base** | the OS | `ubuntu` |
| **Build toolchain** | build an Android app with Flutter | the JDK (headless), the Android SDK (cmdline-tools, platform-tools, build-tools, a platform), `git`, `zip`/`unzip`, `curl`/`wget`, `libstdc++6`, `locales` |
